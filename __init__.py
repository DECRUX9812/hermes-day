...
def _new_rec() -> Dict[str, Any]:
    return {"files": [], "runs": [], "blocks": [], "verdict": None,
            ...
            "gate": []}          # one decision ledger: regex_hit|judged|cached|unjudged
...
def _pre_llm_call(session_id: str = "", user_message: str = "",
    ...
    """
    try:
        # gate context: the user's own words are the mandate the typed phase
        # judges intent_consistent against — captured for every session, not
        # just ones the instincts lane would answer.
        if session_id and (user_message or platform):
            with _LOCK:
                _rec(session_id).update(
                    **({"user_request": str(user_message)[:800]} if user_message else {}),
